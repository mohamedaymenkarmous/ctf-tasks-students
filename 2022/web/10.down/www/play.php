...
if($_COOKIE["PHPSESSID"]==""){
 ...
 echo "Refresh the page to begin. If you have already nothing please activate your cookies.";
  ...
  $dirname="cookies/".$_COOKIE["PHPSESSID"];
//  mkdir("cookies", 0700,true);
//  mkdir($dirname, 0700,true);
//  file_put_contents($dirname."/initGame", "play");
  shell_exec("cp -r test $dirname");
  if(!file_exists($dirname."/initGame")){header("location: play.php");exit();}
?>
<!DOCTYPE html>
<html>
<head>
...
function InitKillers(){
    ...
    $(function() {
        $.ajax({
                url: 'cookies/<?php echo $_COOKIE["PHPSESSID"];?>/killers.json',
                async: false,
                dataType: 'json',
    ...
    $(function() {
        $.ajax({
                url: 'cookies/<?php echo $_COOKIE["PHPSESSID"];?>/killed.json',
                async: false,
                dataType: 'json',
                success: function (killers) {
                var arr=Object.keys(killers);
  ...
  if(followerState[0]==1){//console.log("oui");
    ...
    for(i=0;i<followerID.length;i++){
     if(followerState[i]!=0){
      ...
      if(j==0)$("#revive").show();
                followerState[j]=0;
                followerSize[i]=Kill(followerID[i],followerID[j],followerX[i],followerY[i],followerX[j],followerY[j]);
      getFlag();
                }
           }
          }
